...
-- 4. 上传transformer-0.0.1.jar到hdfs的/alonzo/transformer文件夹中
-- 5. 创建hive的function
create function event_convert as 'com.alonzo.transformer.hive.EventDimensionUDF' using jar 'hdfs://hadoop:8020/alonzo/transformer/transformer-0.0.1.jar';  
...
-- 7. sqoop脚步编写
sqoop export --connect jdbc:mysql://hadoop:3306/report --username hive --password hive --table stats_event --export-dir /hive/bigdater.db/stats_event/* --input-fields-terminated-by "\\01" --update-mode allowinsert --update-key platform_dimension_id,date_dimension_id,event_dimension_id
