...
public class NewMemberRunner extends TransformerBaseRunner {
	...
	private void calculateTotalMembers(Configuration conf) {
		...
		try {
			...
			// 更新操作
			pstmt = conn
					.prepareStatement("INSERT INTO `stats_device_browser`(`platform_dimension_id`,`browser_dimension_id`,`date_dimension_id`,`total_members`) VALUES(?, ?, ?, ?) ON DUPLICATE KEY UPDATE `total_members` = ?");
			for (Map.Entry<String, Integer> entry : oldValueMap.entrySet()) {
